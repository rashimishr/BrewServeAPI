...
namespace BrewServe.Tests.Controllers
{
    ...
    public class BeerControllerTests
    {
        ...
        public void Setup()
        {
            ...
            _controller = new BeerController(_mockBeerService.Object, _mockLogger.Object);
        }
        ...
        {
            // Arrange
            var filter = new BeerFilterCriteria { gtAlcoholByVolume = 4.0m, ltAlcoholByVolume = 7.0m };
           _mockBeerService.Setup(s => s.GetBeersByAlcoholContentAsync(It.Is<BeerFilterCriteria>(f=>
                   f.gtAlcoholByVolume == filter.gtAlcoholByVolume &&
                   f.ltAlcoholByVolume == filter.ltAlcoholByVolume)))
                .ReturnsAsync(new List<BeerResponse>
                {
        ...
        {
            // Arrange
            var filter = new BeerFilterCriteria { gtAlcoholByVolume = 8 };
            _mockBeerService.Setup(s => s.GetBeersByAlcoholContentAsync(filter)).ReturnsAsync(new List<BeerResponse>());
        ...
        public async Task UpdateBeer_ReturnsOkResult_WithUpdatedBeer()
        {
            ...
            _mockBeerService.Setup(service => service.UpdateBeerAsync(beerRequest,1)).ReturnsAsync(updatedBeer);
