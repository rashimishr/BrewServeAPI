...
namespace BrewServe.API.Controllers
{
    ...
    public class BeerController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetBeers([FromQuery] decimal? gtAlcoholByVolume, [FromQuery] decimal? ltAlcoholByVolume)
        {
            ...
            BeerFilterCriteria filter= new BeerFilterCriteria
            {
                gtAlcoholByVolume = gtAlcoholByVolume,
                ltAlcoholByVolume = ltAlcoholByVolume
            };
            var filteredBeers = await _beerService.GetBeersByAlcoholContentAsync(filter);
            if (!filteredBeers.Any() || filteredBeers == null)
            {
                var errorResponse = new ApiResponse<BeerResponse>( Messages.RecordNotFound("Beer"));
                return NotFound(errorResponse);
            }
            var response = new ApiResponse<IEnumerable<BeerResponse>>(filteredBeers);
            return Ok(response);
        }
        ...
        public async Task<IActionResult> UpdateBeer(int id, BeerRequest beer)
        {
            ...
            var updatedBeer = await _beerService.UpdateBeerAsync(beer, id);
